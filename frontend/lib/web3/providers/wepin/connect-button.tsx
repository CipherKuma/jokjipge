...
export function ConnectButton({ className }: ConnectButtonProps) {
  ...
  if (!isWepinConfigured()) {
    return (
      <Button className={className} disabled title="WEPIN not configured">
        {t('wallet.connect')}
      </Button>
    )
  }
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button variant="outline" className={`border-primary/30 hover:border-primary hover:bg-primary/5 ${className || ''}`}>
      ...
      <DropdownMenuContent align="end" className="w-56">
        ...
        >
          ...
          {isDisconnecting ? t('myBets.disconnecting') : t('wallet.disconnect')}
        </DropdownMenuItem>
      </DropdownMenuContent>
    </DropdownMenu>
  )
